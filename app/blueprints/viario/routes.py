...
def browse_dialog(dialog_type: str = "folder") -> str:
    """Abre diálogo nativo do Windows via subprocess — não bloqueia o Flask."""
    if dialog_type == "folder":
        script = (
            "import tkinter as tk; from tkinter import filedialog; "
            "root=tk.Tk(); root.withdraw(); root.wm_attributes('-topmost',True); "
            "print(filedialog.askdirectory(title='Selecione a pasta'))"
        )
    else:
        script = (
            "import tkinter as tk; from tkinter import filedialog; "
...
@viario_bp.route("/api/browse-folder", methods=["POST"])
def api_browse_folder():
    return jsonify({"path": browse_dialog("folder")})


@viario_bp.route("/api/browse-file", methods=["POST"])
def api_browse_file():
    return jsonify({"path": browse_dialog("file")})
...
def api_pipeline_gps():
    pasta_fotos   = request.args.get("pasta", "").strip()
    ...
    def generate():
        try:
            if not pasta_fotos or not os.path.isdir(pasta_fotos):
                yield sse({"type": "error", "msg": f"Pasta não encontrada: {pasta_fotos}"}); return
...
def api_atualizar_km():
    arquivo = request.args.get("arquivo", "").strip()
    rodovia = request.args.get("rodovia", "")
    servico = request.args.get("servico", "")
    ...
    def generate():
        try:
            if not arquivo or not os.path.isfile(arquivo):
                yield sse({"type": "error", "msg": f"Arquivo não encontrado: {arquivo}"}); return
            if rodovia not in _rodovias_runtime:
                yield sse({"type": "error", "msg": f"Rodovia desconhecida: {rodovia}"}); return
            ...
            yield sse({"type": "concluido",
                       "dados": os.path.relpath(arquivo, str(_instance_viario)).replace("\\", "/")})
...
def api_relatorio():
    ...
    def generate():
        try:
            if not arquivo or not os.path.isfile(arquivo):
                yield sse({"type": "error", "msg": "Arquivo de dados não encontrado."}); return
            if not pasta_fotos or not os.path.isdir(pasta_fotos):
                yield sse({"type": "error", "msg": "Pasta de fotos não encontrada."}); return

            df = pd.read_excel(arquivo)
            pasta_dest = os.path.dirname(arquivo)
            ts = datetime.now().strftime("%Y%m%d_%H%M%S")

            yield sse({"type": "etapa", "etapa": 1, "msg": "Gerando relatório .xlsm"})
            arq_rel, err = gerar_relatorio_xlsm(
                df, pasta_fotos, pasta_dest, ts, nome)
            if err:
...
def api_download():
    ...
    full = _instance_viario / filepath.replace("/", os.sep)
    if full.is_file():
